...
[Related Work](#related_work){reference-type="ref" reference="related_work"} section, we
...
domains and preference. The [Data](#data){reference-type="ref"
reference="data"} section goes into detail about the
Manifesto Project Corpus. The [Methodology](#methodology){reference-type="ref"
reference="methodology"} section then introduces our classification approach and provides important details of our models and evaluation approach. In tbe [Results](#results){reference-type="ref" reference="results"} section, we
present our results and address some limitations of our system. Finally,
the [Conclusion](#conclusion){reference-type="ref" reference="conclusion"} summarizes our findings and sets up a roadmap for future improvements.
...
(BERT), with results fine-tuned with party manifestos and
...
::: {#tab:modelspec}
  ...
  BERT-CNN    Base BERT (uncased)  2 Convolutional Layers (1 per filter)2 Max Pooling Layers1 Dropout Layer1 Linear Layer      10
  BERT-GRU    Base BERT (uncased)  1 Bidirectional GRU RNN Layer1 Dropout Layer1 Linear Layer                                  10
...
reference="tab:modelresults"}, the BERT-CNN model
...
We find that using BERT in conjunction with convolutional
